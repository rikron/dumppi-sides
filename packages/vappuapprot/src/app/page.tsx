...
export const metadata: Metadata = {
  title: `Vappuapprot ${metaYear}`,
  description:
    'Vappuapprot on vuosituhannen approt, jotka järjestetään Jyväskylässä.'
}
...
const Page = async () => {
  ...
  const response: any = await fetch(
    `https://ubo8m1s0.api.sanity.io/v2021-03-25/data/query/dumppi-sides`,
    {
      ...
      body: JSON.stringify({
        query:
          "*[_type == 'page' && project=='vappu-approt' && _id=='vappuApproPage'][0] {...,'bannerUrlMobile': bannerMobile.asset -> url,'bannerUrlDesktop': banner.asset -> url,'content': content[]{...,_type=='bannerSection' => {'bannerUrlMobile': bannerMobile.asset -> url,'bannerUrlDesktop': bannerDesktop.asset -> url,},_type=='contactSection' => {...,'content': content[] ->}}}",
        params: {}
      }),
      next: {
        revalidate: 5
